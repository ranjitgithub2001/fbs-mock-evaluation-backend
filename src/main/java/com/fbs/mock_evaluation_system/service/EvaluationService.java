...
public class EvaluationService {
	...
	@Transactional(readOnly = true)
	public SuggestedStageDTO suggestNextStage(Long studentId, Long batchModuleId) {

	    // Validate student and batchModule exist
	    ...
	    int practiceCount = batchModule.getPracticeCount();

	    // Fetch all non-rescheduled evaluations for this student + module, ordered by date
	    List<Evaluation> evaluations = evaluationRepository
	            .findByStudentIdAndBatchModuleIdAndFinalResultNotOrderByMockDateAsc(
	                    studentId, batchModuleId, EvaluationResult.RESCHEDULED);

	    // Separate PRACTICE and non-PRACTICE evaluations
	    List<Evaluation> practiceEvals = new ArrayList<>();
	    List<Evaluation> endModuleEvals = new ArrayList<>();
	    List<Evaluation> pendingEvals = new ArrayList<>();

	    for (Evaluation e : evaluations) {
	        String stageName = e.getMockStage().getName();
	        if ("PRACTICE".equals(stageName)) {
	            practiceEvals.add(e);
	        } else if ("END_MODULE".equals(stageName)) {
	            endModuleEvals.add(e);
	        } else if ("PENDING".equals(stageName)) {
	            pendingEvals.add(e);
	        }
	    }

	    // CASE 1 — No evaluations yet → PRACTICE sequence 1
	    if (evaluations.isEmpty()) {
	        MockStage practiceStage = mockStageRepository.findByName("PRACTICE")
	                .orElseThrow(() -> new ResourceNotFoundException("PRACTICE stage not found"));
	        return new SuggestedStageDTO(practiceStage.getId(), "PRACTICE", 1);
	    }

	    // CASE 2 — Still in PRACTICE phase (attempts < practiceCount)
	    if (practiceEvals.size() < practiceCount) {
	        MockStage practiceStage = mockStageRepository.findByName("PRACTICE")
	                .orElseThrow(() -> new ResourceNotFoundException("PRACTICE stage not found"));
	        int nextSequence = practiceEvals.size() + 1;
	        return new SuggestedStageDTO(practiceStage.getId(), "PRACTICE", nextSequence);
	    }

	    // CASE 3 — All PRACTICE done → move to END_MODULE
	    if (endModuleEvals.isEmpty()) {
	        MockStage endModuleStage = mockStageRepository.findByName("END_MODULE")
	                .orElseThrow(() -> new ResourceNotFoundException("END_MODULE stage not found"));
	        return new SuggestedStageDTO(endModuleStage.getId(), "END_MODULE", null);
	    }

	    // CASE 4 — END_MODULE was CLEARED → module complete
	    Evaluation lastEndModule = endModuleEvals.get(endModuleEvals.size() - 1);
	    if (lastEndModule.getFinalResult() == EvaluationResult.CLEARED) {
	        return new SuggestedStageDTO(null, "MODULE_COMPLETE", null);
	    }

	    // CASE 5 — END_MODULE NOT_CLEARED → PENDING attempt 1
	    if (pendingEvals.isEmpty()) {
	        MockStage pendingStage = mockStageRepository.findByName("PENDING")
	                .orElseThrow(() -> new ResourceNotFoundException("PENDING stage not found"));
	        return new SuggestedStageDTO(pendingStage.getId(), "PENDING", 1);
	    }

	    // CASE 6 — Check last PENDING result
	    Evaluation lastPending = pendingEvals.get(pendingEvals.size() - 1);
	    if (lastPending.getFinalResult() == EvaluationResult.CLEARED) {
	        return new SuggestedStageDTO(null, "MODULE_COMPLETE", null);
	    }

	    // CASE 7 — PENDING NOT_CLEARED → next PENDING attempt
	    MockStage pendingStage = mockStageRepository.findByName("PENDING")
	            .orElseThrow(() -> new ResourceNotFoundException("PENDING stage not found"));
	    int nextPendingAttempt = pendingEvals.size() + 1;
	    return new SuggestedStageDTO(pendingStage.getId(), "PENDING", nextPendingAttempt);
	}
}